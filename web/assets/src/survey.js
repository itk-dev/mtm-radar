// import validation from 'jquery-validation';

(function($) {
    var loadQuestions = function(survey) {
				var questions = survey.find('.question').map(function() {
						var reply = $(this).find('radio');
						;;; console.log(this);
						return {
								title: $(this).find('.title').html(),
								text: $(this).find('.text').html(),
								reply:$(this).find('input[type="radio"]:checked').val(),
								comment: $(this).find('textarea').val(),
						};
				}).get();

				;;; console.log('questions', questions);

				return questions;
		},
				start = function(selector) {
						var survey = $(selector),
								replies = survey.data('replies'),
								configuration = survey.data('configuration'),
								sections = survey.find('section'),
								questions = loadQuestions(survey),
								min = $(selector).data('survey-min'),
								max = (function() {
										var max = 0;
										for (var key in configuration.rating) {
												if (configuration.rating.hasOwnProperty(key)) {
														var value = parseInt(key);
														if (value > max) {
																max = value;
														}
												}
										}

										return max;
								}()),
								index = -1,
								count = sections.length,
								answers = questions.map(function() {
										return null;
								}),
								chart = new Chart(document.getElementById('chart'), {
										type: 'radar',
										data: {
												labels: questions.map(function(question, index) {
														return (index + 1) + '. ' + question.title;
												}),
												datasets: [{
														label: 'Answer',
														fill: false,
														backgroundColor: 'red',
														borderColor: 'blue',
														pointRadius: 8,
														data: [], //questions.map(function() { return null; })
												}]
										},
										options: {
												// responsive: false,
												legend: {
														display: false,
												},
												scale: {
														ticks: {
																// display: false,
																// beginAtZero: true,
																min: 0,
																max: max,
																stepSize: 1
														}
												}
										}
								}),

								updateReply = function(index, reply) {
										chart.data.datasets[0].data[index] = reply;
										chart.update();
										;;; console.log('updateReply', index, reply);
										;;; console.log(chart.data.datasets[0].data[index]);
								},

								showQuestion = function() {
										;;; console.log('showQuestion', index, count);
										if (index < count - 1) {
												sections.hide().eq(index).show();
										} else {
												sections.show();
										}
								},

								prev = function() {
										if (index > 0) {
												index--;
												showQuestion();
										}
								},
								next = function() {
										$('form').validate({
												showErrors: function() {
														$('#validation-errors').toggle(this.errorList.length > 0);
												}
										});
										if ($('form').valid()) {
												if (index < count) {
														// answers[index] = value;
														index++;
												}
												showQuestion();
										}
								};

						if (replies) {
								replies.forEach(function (reply, index) {
										updateReply(index, reply.value);
								});
						} else {
								survey.find('.survey-prev').on('click', prev);
								survey.find('.survey-next').on('click', next);
								survey.find('input[type="radio"]').on('change', function() {
										var index = $(this).closest('.question').data('index');
										;;; console.log('index', index, $(this).val());
										updateReply(index, $(this).val());
								});
								next();
						}
				};

		$(function() {
				start('.survey');
		});
}(jQuery));
